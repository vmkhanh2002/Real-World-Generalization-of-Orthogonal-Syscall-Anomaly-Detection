# real_attack_eval/adduser/scripts/build_manifest.py
"""Scan Adduser_Real capture directory → artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/adduser/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "adduser_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "Adduser_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "Adduser_Real",
            "source_group":         "Adduser_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  Adduser_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "adduser_real_attack_eval",
    "note":           "Victim-side real Adduser traces (sshd strace during useradd session)",
    "entries":        entries,
