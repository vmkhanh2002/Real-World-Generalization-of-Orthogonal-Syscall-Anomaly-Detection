# real_attack_eval/java_meterpreter/scripts/compute_metrics.py
"""
Window-level evaluation for real Java_Meterpreter traces — LOAO-replicated protocol.

LOAO reference numbers are paper constants (not recomputed):
  Java_Meterpreter: AUC=0.9140  AUCPR=0.7654  AUCPR(1:1)=0.8742  F1=0.7726
                    TPR@FPR1%=0.0728  TPR@FPR0.1%=0.0140
"""
import json
import numpy as np
...
LOAO_PAPER = {
    "Java_Meterpreter": {
        "AUC": 0.9140, "AUCPR": 0.7654, "AUCPR_1to1": 0.8742,
        "F1": 0.7726, "TPR@FPR1%": 0.0728, "TPR@FPR01%": 0.0140,
    },
}
...
def print_detection_rate_table(dr, tau):
    ...
    print("=" * W)
    hdr = (f"  {'Family':<26}  {'Traces':>7}  {'Detected':>9}  {'Trace-DR':>9}"
    ...
    for fam in sorted(dr):
        d = dr[fam]
        print(f"  {fam:<26}  {d['n_traces']:>7,}  {d['detected_traces']:>9,}  {d['trace_dr']:>8.1%}"
    ...
    print(f"  {'TOTAL':<26}  {totals['n_traces']:>7,}  {totals['detected']:>9,}  {tdr:>8.1%}"
...
def main():
    ...
    real_pos = load_probs_from_jsonl(REAL_ATK_W20) if REAL_ATK_W20.exists() else np.array([])
    print(f"Real Java_Meterpreter: {len(real_pos):,} windows\n")

    W = 110
    loao = LOAO_PAPER["Java_Meterpreter"]
    print("=" * W)
    ...
    print("=" * W)
    print(f"  {'Source':<34}  {'AUC':>7}  {'AUCPR':>7}  {'AUCPR(1:1)':>10}  {'F1@tau*':>8}  {'TPR@FPR1%':>10}  {'TPR@FPR0.1%':>12}")
    print("-" * W)
    print(f"  {'LOAO Java_Meterpreter (paper)':<34}  {loao['AUC']:>7.4f}  {loao['AUCPR']:>7.4f}  {loao['AUCPR_1to1']:>10.4f}  {loao['F1']:>8.4f}  {loao['TPR@FPR1%']:>10.4f}  {loao['TPR@FPR01%']:>12.4f}")
    ...
    out = {
        ...
        "tau_star":             TAU_STAR,
        "seed":                 SEED,
        "loao_java_meterpreter": loao,
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real Java_Meterpreter':<34}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_java_meterpreter"]    = real_m
        out["detection_rate_by_family"] = dr
    else:
        print()
        print("  NOTE: No real traces — run datasets/java_meterpreter_capture/run_all.sh first")
