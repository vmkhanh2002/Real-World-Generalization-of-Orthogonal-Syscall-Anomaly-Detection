#!/usr/bin/env bash
# run_eval.sh -- Web_Shell eval: manifest → score traces → compute metrics
#
# Usage (from anywhere):
#   bash real_attack_eval/web_shell/scripts/run_eval.sh
#
...
echo "=== [Web_Shell] Build manifest ==="
"$PYTHON" web_shell/scripts/build_manifest.py

echo ""
echo "=== [Web_Shell] Score traces (w=20, i386) ==="
"$PYTHON" scripts/realtime_eval/export_window_scores.py \
    --dataset     32bit \
    --syscall-abi i386 \
    --input-dir   "$CAPTURE_ROOT/webshell_capture/adfa_ld_out/Attack_Data_Master/Web_Shell_Real" \
    --output-jsonl "web_shell/artifacts/window_scores_w20.jsonl" \
    --window-size 20 \
    --stride      2

echo ""
echo "=== [Web_Shell] Compute metrics ==="
"$PYTHON" web_shell/scripts/compute_metrics.py
