# real_attack_eval/web_shell/scripts/build_manifest.py
"""Scan Web_Shell_Real capture directory → artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/web_shell/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "webshell_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "Web_Shell_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "Web_Shell_Real",
            "source_group":         "Web_Shell_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  Web_Shell_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "webshell_real_attack_eval",
    "note":           "Victim-side real Web_Shell traces (apache2 strace during DVWA webshell execution)",
    "entries":        entries,
