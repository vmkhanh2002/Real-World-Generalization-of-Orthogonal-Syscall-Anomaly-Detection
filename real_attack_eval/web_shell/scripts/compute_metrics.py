# real_attack_eval/web_shell/scripts/compute_metrics.py
"""
Window-level evaluation for real Web_Shell traces — LOAO-replicated protocol.

LOAO reference numbers are paper constants (not recomputed):
  Web_Shell: AUC=0.9073  AUCPR=0.7772  AUCPR(1:1)=0.8814  F1=0.7421
             TPR@FPR1%=0.1087  TPR@FPR0.1%=0.0167
"""
import json
import numpy as np
...
LOAO_PAPER = {
    "Web_Shell": {
        "AUC": 0.9073, "AUCPR": 0.7772, "AUCPR_1to1": 0.8814,
        "F1": 0.7421, "TPR@FPR1%": 0.1087, "TPR@FPR01%": 0.0167,
    },
}
...
def main():
    ...
    print(f"Real Web_Shell: {len(real_pos):,} windows\n")
    real_m = compute_metrics(real_pos, neg_test, TAU_STAR)

    W = 110
    loao = LOAO_PAPER["Web_Shell"]
    print("=" * W)
    ...
    print("-" * W)
    print(f"  {'LOAO Web_Shell (paper)':<30}  {loao['AUC']:>7.4f}  {loao['AUCPR']:>7.4f}  {loao['AUCPR_1to1']:>10.4f}  {loao['F1']:>8.4f}  {loao['TPR@FPR1%']:>10.4f}  {loao['TPR@FPR01%']:>12.4f}")
    print(f"  {'Real Web_Shell':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
    print()
