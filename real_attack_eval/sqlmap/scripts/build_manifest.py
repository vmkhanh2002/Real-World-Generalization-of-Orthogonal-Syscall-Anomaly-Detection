# real_attack_eval/sqlmap/scripts/build_manifest.py
"""Scan SQLMap_Real capture directory → artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/sqlmap/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "sqlmap_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "SQLMap_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "SQLMap_Real",
            "source_group":         "SQLMap_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  SQLMap_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "sqlmap_real_attack_eval",
    "note":           "Victim-side real SQLMap traces (apache2 strace, SQLi against vulnerable PHP/SQLite page)",
    "entries":        entries,
