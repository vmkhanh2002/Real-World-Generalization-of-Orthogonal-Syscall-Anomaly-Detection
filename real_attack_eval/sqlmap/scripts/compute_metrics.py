# real_attack_eval/sqlmap/scripts/compute_metrics.py
"""
Window-level evaluation for real SQLMap traces — LOAO-replicated protocol.

SQLMap: NOVEL family (not in ADFA-LD training) — no LOAO baseline.
Label: real_sqlmap
"""
import json
import numpy as np
...
def main():
    print("SQLMap: NOVEL family (not in ADFA-LD training) — no LOAO baseline")
    print()
    ...
    print(f"Real SQLMap: {len(real_pos):,} windows\n")
    ...
    out = {
        ...
        "tau_star":          TAU_STAR,
        "seed":              SEED,
        "note":              "SQLMap: NOVEL family (not in ADFA-LD training) — no LOAO baseline",
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real SQLMap':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        print("  No LOAO row: SQLMap is a novel family not present in ADFA-LD training data.")
        ...
        out["real_sqlmap"]               = real_m
        out["detection_rate_by_family"]  = dr
    else:
        print()
        print("  NOTE: No real traces — run datasets/sqlmap_capture/run_all.sh first")
