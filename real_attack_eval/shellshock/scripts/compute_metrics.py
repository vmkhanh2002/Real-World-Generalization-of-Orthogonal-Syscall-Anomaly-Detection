# real_attack_eval/shellshock/scripts/compute_metrics.py
"""
Window-level evaluation for real Shellshock traces -- LOAO-replicated protocol.

Novel family: Shellshock (CVE-2014-6271) was not part of the ADFA-LD training set.
No LOAO paper constants exist for this family -- real_shellshock row only.
"""
import json
import numpy as np
...
def main():
    ...
    print(f"Real Shellshock: {len(real_pos):,} windows\n")
    ...
    print(f"  Novel family: no LOAO paper constants (Shellshock not in ADFA-LD training set)")
    ...
    out = {
        ...
        "tau_star":          TAU_STAR,
        "seed":              SEED,
        "note":              "Novel family: Shellshock not in ADFA-LD; no LOAO paper constants",
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real Shellshock':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_shellshock"]          = real_m
        out["detection_rate_by_family"] = dr
    else:
        print()
        print("  NOTE: No real traces -- run datasets/shellshock_capture/run_all.sh first")
