# real_attack_eval/shellshock/scripts/build_manifest.py
"""Scan Shellshock_Real capture directory -> artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/shellshock/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "shellshock_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "Shellshock_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "Shellshock_Real",
            "source_group":         "Shellshock_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  Shellshock_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "shellshock_real_attack_eval",
    "note":           "Victim-side real Shellshock traces (apache2 strace during CVE-2014-6271 CGI exploitation)",
    "entries":        entries,
