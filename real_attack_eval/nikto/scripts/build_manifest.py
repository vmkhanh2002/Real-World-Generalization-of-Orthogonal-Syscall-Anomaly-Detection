# real_attack_eval/nikto/scripts/build_manifest.py
"""Scan Nikto_Real capture directory → artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/nikto/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "nikto_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "Nikto_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "Nikto_Real",
            "source_group":         "Nikto_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  Nikto_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "nikto_real_attack_eval",
    "note":           "Victim-side real Nikto traces (apache2 strace during Nikto web vulnerability scan)",
    "entries":        entries,
