# real_attack_eval/nikto/scripts/compute_metrics.py
"""
Window-level evaluation for real Nikto traces — LOAO-replicated protocol.

Nikto is a novel attack family (web vulnerability scanner) not present in the
ADFA-LD training set. No LOAO paper constants exist for this family.
Metrics measure zero-shot generalization of the trained anomaly detector.
"""
import json
import numpy as np
...
def main():
    ...
    print(f"Real Nikto: {len(real_pos):,} windows\n")
    ...
    print(f"  Novel family: no LOAO paper constants. Zero-shot generalization measurement.")
    print("=" * W)
    ...
    out = {
        ...
        "tau_star":        TAU_STAR,
        "seed":            SEED,
        "note":            "Novel family — no LOAO paper constants. Zero-shot generalization.",
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real Nikto':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_nikto"]                = real_m
        out["detection_rate_by_family"]  = dr
    else:
        print()
        print("  NOTE: No real traces — run datasets/nikto_capture/run_all.sh first")
