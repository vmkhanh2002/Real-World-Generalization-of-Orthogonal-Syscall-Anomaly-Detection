# real_attack_eval/reverse_shell/scripts/compute_metrics.py
"""
Window-level evaluation for real Reverse Shell traces — LOAO-replicated protocol.

ReverseShell: NOVEL family (not in ADFA-LD training) — no LOAO baseline.
"""
import json
import numpy as np
...
def main():
    print("ReverseShell: NOVEL family (not in ADFA-LD training) — no LOAO baseline")
    print()
    ...
    print(f"Real Reverse Shell: {len(real_pos):,} windows\n")
    ...
    if len(real_pos) > 0:
        ...
        print(f"  {'Real Reverse Shell':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_reverse_shell"]        = real_m
        out["detection_rate_by_family"]  = dr
    else:
        print()
        print("  NOTE: No real traces — run datasets/reverse_shell_capture/run_all.sh first")
