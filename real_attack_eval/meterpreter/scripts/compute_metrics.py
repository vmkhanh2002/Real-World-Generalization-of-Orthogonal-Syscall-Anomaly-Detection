# real_attack_eval/meterpreter/scripts/compute_metrics.py
"""
Window-level evaluation for real Meterpreter traces — LOAO-replicated protocol.

LOAO reference numbers are paper constants (not recomputed):
  Meterpreter: AUC=0.9140  AUCPR=0.6985  AUCPR(1:1)=0.8933  F1=0.6441
               TPR@FPR1%=0.1297  TPR@FPR0.1%=0.0220
"""
import json
import numpy as np
...
LOAO_PAPER = {
    "Meterpreter": {
        "AUC": 0.9140, "AUCPR": 0.6985, "AUCPR_1to1": 0.8933,
        "F1": 0.6441, "TPR@FPR1%": 0.1297, "TPR@FPR01%": 0.0220,
    },
}
...
def main():
    ...
    print(f"Real Meterpreter: {len(real_pos):,} windows\n")

    W = 110
    loao = LOAO_PAPER["Meterpreter"]
    print("=" * W)
    ...
    print("-" * W)
    print(f"  {'LOAO Meterpreter (paper)':<30}  {loao['AUC']:>7.4f}  {loao['AUCPR']:>7.4f}  {loao['AUCPR_1to1']:>10.4f}  {loao['F1']:>8.4f}  {loao['TPR@FPR1%']:>10.4f}  {loao['TPR@FPR01%']:>12.4f}")
    ...
    out = {
        ...
        "tau_star":           TAU_STAR,
        "seed":               SEED,
        "loao_meterpreter":   loao,
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real Meterpreter':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_meterpreter"]          = real_m
        out["detection_rate_by_family"]  = dr
    else:
        print()
        print("  NOTE: No real traces — run datasets/meterpreter_capture/run_all.sh first")
