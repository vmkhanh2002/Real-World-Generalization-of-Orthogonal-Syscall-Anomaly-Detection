# real_attack_eval/meterpreter/scripts/build_manifest.py
"""Scan Meterpreter_Real capture directory → artifacts/manifest.json.
...
_FAMILY_DIR    = _HERE.parent                         # real_attack_eval/meterpreter/
...
CAPTURE_BASE = Path(os.environ.get("CAPTURE_ROOT", str(_default_capture))) \
               / "meterpreter_capture" / "adfa_ld_out" / "Attack_Data_Master"

entries = []
d = CAPTURE_BASE / "Meterpreter_Real"
if not d.exists():
    ...
    for f in sorted(d.glob("*.txt")):
        ...
        entries.append({
            ...
            "target_preset":        "Meterpreter_Real",
            "source_group":         "Meterpreter_Real",
            "source_file":          source_file,
            "expected_detect_file": source_file,
        })
    print(f"  Meterpreter_Real: {len(entries)} traces")

OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
OUT_PATH.write_text(json.dumps({
    "schema_version": 2,
    "manifest_type":  "meterpreter_real_attack_eval",
    "note":           "Victim-side real Meterpreter traces (vsftpd strace, CVE-2011-2523 backdoor)",
    "entries":        entries,
