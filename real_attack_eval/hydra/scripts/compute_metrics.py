# real_attack_eval/hydra/scripts/compute_metrics.py
"""
Window-level evaluation for real Hydra traces — LOAO-replicated protocol.

LOAO reference numbers are paper constants (not recomputed):
  Hydra FTP: AUC=0.9112  AUCPR=0.7380  AUCPR(1:1)=0.8786  F1=0.7189
             TPR@FPR1%=0.0909  TPR@FPR0.1%=0.0092
  Hydra SSH: AUC=0.8970  AUCPR=0.7907  AUCPR(1:1)=0.8694  F1=0.7695
             TPR@FPR1%=0.1366  TPR@FPR0.1%=0.0094
"""
import json
import numpy as np
...
_HYDRA_DIR  = _HERE.parent
_SHARED_DIR = _HYDRA_DIR.parent / "shared"

REAL_ATK_W20  = _HYDRA_DIR  / "artifacts" / "window_scores_w20.jsonl"
ADFA_BENIGN   = _SHARED_DIR / "adfa_benign_w20.jsonl"
REAL_MANIFEST = _HYDRA_DIR  / "artifacts" / "manifest.json"

# Paper constants — never recomputed, used only as reference rows
LOAO_PAPER = {
    "Hydra_FTP": {"AUC": 0.9112, "AUCPR": 0.7380, "AUCPR_1to1": 0.8786,
                  "F1": 0.7189, "TPR@FPR1%": 0.0909, "TPR@FPR01%": 0.0092},
    "Hydra_SSH": {"AUC": 0.8970, "AUCPR": 0.7907, "AUCPR_1to1": 0.8694,
                  "F1": 0.7695, "TPR@FPR1%": 0.1366, "TPR@FPR01%": 0.0094},
    "Average":   {"AUC": 0.9081, "AUCPR": 0.7421, "AUCPR_1to1": 0.8774,
                  "F1": 0.7255, "TPR@FPR1%": 0.1032, "TPR@FPR01%": 0.0138},
}
...
def load_family_probs(jsonl_path, manifest_path, family_filter):
    """Load phase5 probabilities for traces matching family_filter in manifest."""
    manifest = json.loads(manifest_path.read_text())
    family_files = {
        e["source_file"].replace("\\", "/")
        for e in manifest["entries"]
        if family_filter in str(e.get("target_preset", ""))
    }
    probs = []
    for line in jsonl_path.open(encoding="utf-8"):
        ...
        if not line:
            continue
        row = json.loads(line)
        if row["file"].replace("\\", "/") in family_files:
            probs.extend(row.get("phase5_probs", []))
...
def avg_metrics(a, b):
    keys = ["AUC", "AUCPR", "AUCPR_1to1", "F1", "TPR@FPR1%", "TPR@FPR01%"]
    result = {k: round(np.mean([a[k], b[k]]), 4) for k in keys}
    result["n_pos"] = a["n_pos"] + b["n_pos"]
    result["n_neg"] = a["n_neg"]
    return result
...
def print_detection_rate_table(dr, tau):
    ...
    print("  SSH Win-DR lower: DH key-exchange creates many crypto windows that score benign")
...
def main():
    ...
    print("Loading real attack windows...")
    real_ftp = load_family_probs(REAL_ATK_W20, REAL_MANIFEST, "Hydra_FTP")
    real_ssh = load_family_probs(REAL_ATK_W20, REAL_MANIFEST, "Hydra_SSH")
    print(f"  Real: FTP={len(real_ftp):,}  SSH={len(real_ssh):,} windows\n")

    real_ftp_m = compute_metrics(real_ftp, neg_test, TAU_STAR)
    real_ssh_m = compute_metrics(real_ssh, neg_test, TAU_STAR)
    real_avg_m = avg_metrics(real_ftp_m, real_ssh_m)
    ...
    for label, m, kind in [
        ("LOAO Hydra FTP (paper)", LOAO_PAPER["Hydra_FTP"], "loao"),
        ("Real Hydra FTP",         real_ftp_m,              "real"),
        ("",                       None,                     None),
        ("LOAO Hydra SSH (paper)", LOAO_PAPER["Hydra_SSH"], "loao"),
        ("Real Hydra SSH",         real_ssh_m,              "real"),
        ("",                       None,                     None),
        ("LOAO Average (paper)",   LOAO_PAPER["Average"],   "loao"),
        ("Real Average",           real_avg_m,              "real"),
    ]:
        if not label:
            print()
            continue
        aucpr = f"{m['AUCPR']:>7.4f}" if kind != "real" else f"{'n/a':>7}"
        f1    = f"{m['F1']:>8.4f}"    if kind != "real" else f"{'n/a':>8}"
        print(f"  {label:<30}  {m['AUC']:>7.4f}  {aucpr}  {m['AUCPR_1to1']:>10.4f}  {f1}  {m['TPR@FPR1%']:>10.4f}  {m['TPR@FPR01%']:>12.4f}")

    print()
    print("  LOAO rows: paper constants — not recomputed.")
    ...
    out = {
        ...
        "tau_star":          TAU_STAR,
        "seed":              SEED,
        "loao_hydra_ftp":    LOAO_PAPER["Hydra_FTP"],
        "loao_hydra_ssh":    LOAO_PAPER["Hydra_SSH"],
        "loao_hydra_avg":    LOAO_PAPER["Average"],
        "real_hydra_ftp":    real_ftp_m,
        "real_hydra_ssh":    real_ssh_m,
        "real_hydra_avg":    real_avg_m,
        "detection_rate_by_family": dr,
    }
    out_path = _HYDRA_DIR / "artifacts" / "window_metrics_w20.json"
