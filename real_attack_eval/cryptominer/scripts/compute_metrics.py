# real_attack_eval/cryptominer/scripts/compute_metrics.py
"""
Window-level evaluation for real Cryptominer traces — LOAO-replicated protocol.

Cryptominer: NOVEL family (not present in ADFA-LD training set).
No LOAO paper constants — purely prospective evaluation of cross-family generalization.
"""
import json
import numpy as np
...
def main():
    print("Cryptominer: NOVEL family (not in ADFA-LD training set)")
    print("Prospective cross-family generalization evaluation.\n")
    ...
    print(f"Real Cryptominer: {len(real_pos):,} windows\n")
    ...
    out = {
        ...
        "tau_star":           TAU_STAR,
        "seed":               SEED,
        "note":               "Cryptominer: NOVEL family — no LOAO paper constants",
    }

    if len(real_pos) > 0:
        real_m = compute_metrics(real_pos, neg_test, TAU_STAR)
        print(f"  {'Real Cryptominer':<30}  {real_m['AUC']:>7.4f}  {'n/a':>7}  {real_m['AUCPR_1to1']:>10.4f}  {'n/a':>8}  {real_m['TPR@FPR1%']:>10.4f}  {real_m['TPR@FPR01%']:>12.4f}")
        print()
        ...
        out["real_cryptominer"]          = real_m
        out["detection_rate_by_family"]  = dr
    else:
        print()
        print("  NOTE: No real traces -- run datasets/cryptominer_capture/run_all.sh first")
